	...
	public static async Task<(int total, Dictionary<int, SongIdentifier> songs)> GetAllSongsFromRunLogFile(string path)
	{
		...
		for (int i = 0; i < allFileLinesNumbered.Count; i++) {
			...
			if (Regex.IsMatch(val, _songDeclarePattern)) {
				...
				Console.WriteLine($"First match value = {Regex.Match(val, _songDeclarePattern).Captures[0].Value}, Regex Capture count = {Regex.Match(val, _songDeclarePattern).Captures.Count}, ");
