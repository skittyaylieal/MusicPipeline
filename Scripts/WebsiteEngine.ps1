...
# -----------------------------------------------------------------
# 1. FULL BACKGROUND SCANNER WITH ARCHIVE METADATA BUILDER
# -----------------------------------------------------------------
function Start-AsyncLibraryScanner {
    ...
    $JobScript = {
        ...
        while ($true) {
            ...
            # Rebuilding your complete Track Navigation Database
            $TrackDatabase = @()
            ...
            # Drop the complete package layout to file cache
            @{
...
# Full HTML Dashboard Asset Restored
$HtmlDashboard = @'
<!DOCTYPE html>
<html lang="en">
<head>
...
# -----------------------------------------------------------------
# 3. CORE WEB ENGINE LISTENER ROUTER LOOP (WITH EXPLICIT HEADER CLOSE)
# -----------------------------------------------------------------
$Listener = New-Object System.Net.HttpListener
$Listener.Prefixes.Add("http://localhost:8080/")
...
try {
    ...
    while ($true) {
        try {
            ...
            # THE MAGIC FIX: Explicitly shut down the channel immediately after transmission
            $Response.KeepAlive = $false
            $Response.Headers.Add("Connection", "close")
