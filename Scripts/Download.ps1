...
# --- PROTON VPN DESKTOP APPLICATION LIFECYCLE ---
$ProtonVpnPath = "C:\Program Files\Proton\VPN\ProtonVPN.exe"
...
try {
    ...
    # Optimization: Parallel Playlist Auditing (Sequenced cleanly via ThrottleLimit)
    $SanitizedURLs | ForEach-Object -Parallel {
        ...
        try {
            ...
            $YTDLArgs = @(
                ...
                "-f", "bestaudio/best",              # Capture the absolute highest bitrate track
    ...
    # --- CLEANUP: CLOSE PROTON VPN ON EXIT ---
    if ($VpnLaunched) {
        Write-Output "============================================="
        Write-Output "[VPN] Terminating Proton VPN Desktop Application..."
        Write-Output "============================================="
        Stop-Process -Name $VpnProcessName -Force -ErrorAction SilentlyContinue
    }
